...
# @Use: Given a dictionary mapping word pairs to frequencies, output
# an instance of pulp milp class
...
def milp(scores):
  ...
  # objective function
  obj  = [ (w[ij] - s[ij]) * scores[ij] for ij in wwords if iNotj(ij)    ] \
       + [ (w[ii] + s[ii]) * -C         for ii in wwords if not iNotj(ii)]
...
# toWords :: Dict String Int -> [String]
def toWords(scores):
